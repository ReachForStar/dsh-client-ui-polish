...
- **带费用的会话统计浮窗。** 一个 `conversation.composer.dock` 条目以 `position: fixed` 钉在视口右上角，展示持久的 `sessionStats` 与 `tokenUsage` 投影数字（未组合前者时回退到窗口折叠），并附加按固定 DeepSeek 价目表（输入 1.5、输出 4.5、缓存读取 0.05 元／百万 token）估算的花费，悬停显示各桶拆分。
