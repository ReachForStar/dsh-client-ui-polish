...
- **文件面板。** 一个 `conversation.view` 标签页（位于「轨迹」与 Git 标签之间），列出会话中被工具操作过的所有文件（read／edit／write 卡片）。选中文件后通过宿主的 `/git/read` 路由读取当前内容到可编辑文本框；保存经 `/git/write` 原地写回——文件在面板内编辑，绝不交给第三方应用。
