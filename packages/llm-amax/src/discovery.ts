/**
 * Answering "which models can the AMAX gateway serve?" for the configuration
 * surface's "fetch available models" action. The AMAX provider ships no
 * static catalog — the router's models depend on the account's token plan —
 * so the route is always interrogated over the wire against the
 * OpenAI-compatible `GET /models` endpoint, with the gateway's own base URL
 * as the fallback when the draft carries none.
 *
...
import { AMAX_PROVIDER } from './catalog.ts'
...
/**
 * Interrogate the AMAX gateway for the models it advertises.
...
export async function discoverModels(
  ...
  // The gateway's own endpoint is the fallback: the configuration surface can
  // interrogate the route before any base URL is typed, because the catalog
  // entry declares it. Only a route without that fallback is refused here.
  const baseURL = request.baseURL ?? (request.provider === AMAX_PROVIDER.id ? AMAX_PROVIDER.baseUrl : undefined)
  if (baseURL === undefined || baseURL.length === 0) {
    throw new LlmError(
      `no endpoint for provider "${request.provider ?? ''}"; set a baseURL, or enter this provider's models by hand`,
      'DISCOVERY_FAILED',
    )
  }
