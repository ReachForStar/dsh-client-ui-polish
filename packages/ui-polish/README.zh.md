...
- **内嵌 Excalidraw 白板标签页。** 一个 `conversation.view` 标签页渲染工作区场景文件（`<workspace>/.dsh/excalidraw/scene.json`，与 `@deepseek-ai/dsh-tool-excalidraw` 模型工具共用的 `SCENE_RELATIVE`）为实时画布，跟随 DSH 主题，支持 PNG/SVG 导出。node 半侧提供 `/scene` 路由；模型的 `excalidraw_*` 工具编辑同一文件，因此模型画的形状实时出现在白板上。
