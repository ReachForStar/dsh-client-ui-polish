...
安装本 bundle（`dsh plugin --profile web add ./packages/subagent-pi`）后，提供方在宿主机加载一次，在工具调用前不启动任何 Pi 进程。模型面向的委派工具按 agent 授予：本仓库的 `presets/standard-polished` preset 携带已启用的 `tool-subagent-pi` 行，或者把下面两行加入现有 preset（复制 preset 并从工具行移除 `disabled`，与 codex/claude-code 行完全一致）。
