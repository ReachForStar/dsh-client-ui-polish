...
- **Session stats float with cost.** A `conversation.composer.dock` entry pinned to the viewport's top-right via `position: fixed` shows the durable `sessionStats` and `tokenUsage` projection figures (window-fold fallback for assemblies without the former), plus an estimated spend billed per model: each settled assistant step's usage is priced at its own model's rate from the editable rate card in `src/client/model-pricing.json` (CNY per 1M tokens, converted once from the amaxsmp gateway pricing), with a hover breakdown per bucket. Unknown models and sessions without attributable node usage fall back to the `default` card (deepseek-v4-flash: input 1.5, output 4.5, cache-read 0.05); edit the JSON and rebuild to update prices.
