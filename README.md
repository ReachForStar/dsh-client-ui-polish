...
- **Git panel.** A third `conversation.composer.dock` entry toggles a fixed panel over the repository the host process runs in: current branch, working-tree changes with per-file diffs, a commit box (`add -A` + commit), and a push action. The node half registers `/git/*` routes on the host webserver and runs `git` through `execFile` with array arguments (no shell), so paths and messages never reach a shell; the browser half is a plain fetch client. Paths containing `..` or separators are rejected, and a non-repo cwd shows a quiet notice.
